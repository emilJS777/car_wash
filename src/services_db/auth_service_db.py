...
# GENERATE TOKENS DB
def generate_pair_token(user_id: int) -> Auth:
    auth: Auth = Auth.query.filter_by(user_id=user_id).first() or Auth(user_id=user_id)
...
# GET AUTH BY USER ID
def get_auth_by_user_id(user_id: int) -> Auth:
