...
def get_device_error_ids() -> list:
    ids = list([])
    device_errors = list(DeviceError.query.order_by(DeviceError.creation_date).all())
...
def get_device_error_by_id(device_error_id) -> DeviceError:
    device_error = DeviceError.query.filter_by(id=device_error_id).first()
    return device_error


# CREATE DEVICE ERROR
def create_device_error(device_code, msg) -> DeviceError:
